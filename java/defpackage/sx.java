...
class sx {
    ...
    private void a(byte[] bytes) {
        ...
        byte[] timeBytes = sx.a(time / 1000);
        bytes[0] = timeBytes[0];
        bytes[1] = timeBytes[1];
        bytes[2] = timeBytes[2];
        bytes[3] = timeBytes[3];
        byte[] msecsBytes = sx.b(tvUsec);
        bytes[4] = msecsBytes[0];
        bytes[5] = msecsBytes[1];
    }

    private void b(byte[] bytes) {
        byte[] sequenceBytes = sx.b(a.incrementAndGet());
        bytes[6] = sequenceBytes[0];
        bytes[7] = sequenceBytes[1];
    }

    private void c(byte[] bytes) {
        byte[] pidBytes = sx.b((long) Integer.valueOf(Process.myPid()).shortValue());
        bytes[8] = pidBytes[0];
        bytes[9] = pidBytes[1];
    }
