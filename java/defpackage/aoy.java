...
public abstract class aoy {
    ...
    abstract void a(byte[] bArr, int i, int i2, aoy$a aoy_a);

    protected abstract boolean a(byte b);

    abstract void b(byte[] bArr, int i, int i2, aoy$a aoy_a);

    protected aoy(int unencodedBlockSize, int encodedBlockSize, int lineLength, int chunkSeparatorLength) {
        boolean useChunking;
        int i = 0;
    ...
    protected byte[] a(int size, aoy$a context) {
    ...
    int c(byte[] b, int bPos, int bAvail, aoy$a context) {
        if (context.c != null) {
