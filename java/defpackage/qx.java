...
public class qx extends oh {
    private oh a;
    private qx$a b;
    private boolean c;

    public qx(oh wrapped, int side) {
        this(new qx$a(wrapped.getConstantState(), side), wrapped, null);
    }

    qx(qx$a state, oh wrapped, Resources res) {
        this.b = state;
        if (wrapped != null) {
            this.a = wrapped;
        } else if (res != null) {
            this.a = (oh) state.a.newDrawable(res);
        } else {
            this.a = (oh) state.a.newDrawable();
        }
    }
