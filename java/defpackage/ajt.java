...
public class ajt {
    ...
    public static Date a(String date, ParsePosition pos) throws ParseException {
        ...
        try {
            ...
            int i = index + 4;
            int year = ajt.a(date, index, i);
            if (ajt.a(date, i, '-')) {
                i++;
            }
            index = i + 2;
            int month = ajt.a(date, i, index);
            if (ajt.a(date, index, '-')) {
                i = index + 1;
            } else {
                i = index;
            ...
            index = i + 2;
            int day = ajt.a(date, i, index);
            int hour = 0;
            int minutes = 0;
            int seconds = 0;
            int milliseconds = 0;
            boolean hasT = ajt.a(date, index, 'T');
            Calendar calendar;
            if (hasT || date.length() > index) {
                if (hasT) {
                    index++;
                    i = index + 2;
                    hour = ajt.a(date, index, i);
                    if (ajt.a(date, i, ':')) {
                        i++;
                    }
                    index = i + 2;
                    minutes = ajt.a(date, i, index);
                    if (ajt.a(date, index, ':')) {
                        i = index + 1;
                    } else {
                        i = index;
                    ...
                    if (date.length() > i) {
                        ...
                        if (!(c == 'Z' || c == '+' || c == '-')) {
                            index = i + 2;
                            seconds = ajt.a(date, i, index);
                            if (seconds > 59 && seconds < 63) {
                                seconds = 59;
                            }
                            if (ajt.a(date, index, '.')) {
                                index++;
                                int endOffset = ajt.a(date, index + 1);
                                int parseEndOffset = Math.min(endOffset, index + 3);
                                int fraction = ajt.a(date, index, parseEndOffset);
