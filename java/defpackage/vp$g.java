...
final class vp$g {
    ...
    static JSONObject a(InputStream stream) throws IOException {
        ...
        for (int i = 0; i < 3; i++) {
            int b = stream.read();
            if (b == -1) {
                vx.a(LoggingBehavior.CACHE, vp.a, "readHeader: stream.read returned -1 while reading header size");
                return null;
            }
        ...
        while (count < headerBytes.length) {
            ...
            if (readCount < 1) {
                vx.a(LoggingBehavior.CACHE, vp.a, "readHeader: stream.read stopped at " + Integer.valueOf(count) + " when expected " + headerBytes.length);
                return null;
            }
            count += readCount;
        }
        try {
            ...
            vx.a(LoggingBehavior.CACHE, vp.a, "readHeader: expected JSONObject, got " + parsed.getClass().getCanonicalName());
            return null;
