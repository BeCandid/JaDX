...
class ane implements X509TrustManager {
    ...
    public ane(anf keyStore, and pinningInfoProvider) {
        this.b = a(keyStore);
        this.c = keyStore;
    ...
    private TrustManager[] a(anf keyStore) {
        try {
    ...
    private void a(X509Certificate[] chain) throws CertificateException {
        if (this.d == -1 || System.currentTimeMillis() - this.d <= 15552000000L) {
            X509Certificate[] arr$ = amz.a(chain, this.c);
            int len$ = arr$.length;
            int i$ = 0;
            while (i$ < len$) {
                if (!a(arr$[i$])) {
        ...
        akx.h().d("Fabric", "Certificate pins are stale, (" + (System.currentTimeMillis() - this.d) + " millis vs " + 15552000000L + " millis) " + "falling back to system trust.");
    }
