...
public class im {
    ...
    public static Bitmap a(Bitmap source, int finalWidth, int finalHeight) {
        float destAspect = ((float) finalWidth) / ((float) finalHeight);
        float srcAspect = ((float) source.getWidth()) / ((float) source.getHeight());
        int width = source.getWidth();
        int height = (int) (((float) source.getWidth()) / destAspect);
        try {
            Bitmap bm = im.a(Bitmap.createBitmap(source, 0, (source.getHeight() - height) / 2, width, height), width / 32, true);
            Canvas canvas = new Canvas(bm);
            Paint p = new Paint(-65536);
            p.setColorFilter(new LightingColorFilter(-9408400, 1118481));
